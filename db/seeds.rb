...
(2..24).each do |number|
  ...
  Computer.create(name: "pc_301_#{number}", owner: true, mac_address: "AA:AA:BB:BB:CC:#{number+10}", ip: "192.168.0.#{50 + number}", room: room_301, account: udesc)
end
...
(2..24).each do |number|
  ...
  Computer.create(name: "pc_302_#{number}", owner: true, mac_address: "AA:AA:BB:BB:#{number+10}:C1", ip: "192.168.0.#{100 + number}", room: room_302, account: udesc)
end
...
(2..24).each do |number|
  ...
  Computer.create(name: "pc_302_3#{number}", owner: true, mac_address: "AA:AA:BB:BB:#{number+50}:C1", ip: "192.168.0.#{150 + number}", room: room_302, account: udesc)
end
